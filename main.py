...
pygame.display.set_caption('Little Marcelo')
...
def menu():
  ...
  pygame.mixer.music.load(os.path.join(diretorio_musicas, 'playlist.mp3'))
...
#Tela final
def tela_final():
  tela.blit(MENU_FINAL, (0,0))
  score_total = exibe_mensagem(f'{total}', 20, ('black'))
  tela.blit(score_total, (327, 211))
  pygame.display.flip()
  pressionar_espaço()
  if run == True:
    reiniciar_tudo()
...
MENU_FINAL = pygame.image.load(os.path.join(diretorio_imagens, 'menu_final.png'))
MENU_INICIAL = pygame.image.load(os.path.join(diretorio_imagens, 'menu_inicial.png'))
...
def reiniciar_tudo():
  global pontos, velocidade_jogo, colidiu, escolha_obstaculo, pizzaa, cafee, coxinhaa, move, move_2, game_over_count, lifex_1, lifex_2, lifex_3, flag, game_speed, obstacles
  velocidade_jogo = 10
  colidiu = False
  estagiario.rect.y = altura - 64 - 96 // 2
  estagiario.pulo = False
...
while tecla_espaço:
  ...
  #Background inicio
  tela_fundo = pygame.image.load(os.path.join(diretorio_imagens, 'background.jpg'))
  background = pygame.transform.scale(tela_fundo, (1280, 450))
  if flag == 0:
  ...
  for event in pygame.event.get():
    ...
    if event.type == KEYDOWN:
      if event.key == K_SPACE:
        estagiario.pular()
      if event.key == K_SPACE and estagiario.colisao(guardinha) and game_over_count < 2:
        reiniciar()
        flag = 0
        game_over_count += 1
      elif event.key == K_SPACE and estagiario.colisao(guardinha) and game_over_count == 2:
        reiniciar_tudo()
